...
exports.mapWmoCodeToWeatherType = mapWmoCodeToWeatherType;
 ...
 */
function mapWmoCodeToWeatherType(code, isDay = true) {
    ...
    if (code === 2)
        return { type: "cloudy", description: "Partly cloudy" };
    if (code === 3)
        return { type: "cloudy", description: "Overcast" };
    if (code === 45 || code === 48)
        return { type: "cloudy", description: "Foggy" };
 ...
 */
async function fetchLiveWeather(city, override) {
    const normalizedOverride = (override || "").toLowerCase().trim();
    if (normalizedOverride === "sunny" ||
    ...
    try {
        ...
        const geoUrl = `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(queryCity)}&count=1`;
