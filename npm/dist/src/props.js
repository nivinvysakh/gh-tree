...
exports.renderMinecraftGround = renderMinecraftGround;
exports.renderMinecraftLog = renderMinecraftLog;
exports.renderMinecraftLeaf = renderMinecraftLeaf;
exports.renderFlowerOnGrass = renderFlowerOnGrass;
exports.renderApple = renderApple;
exports.renderGoldenAppleOnGrass = renderGoldenAppleOnGrass;
exports.renderMinecraftBee = renderMinecraftBee;
exports.renderMinecraftBeehive = renderMinecraftBeehive;
exports.renderMinecraftSignpost = renderMinecraftSignpost;
exports.renderMinecraftFarmer = renderMinecraftFarmer;
exports.renderMinecraftWolf = renderMinecraftWolf;
exports.renderMinecraftFox = renderMinecraftFox;
exports.renderMinecraftCat = renderMinecraftCat;
exports.renderMinecraftParrot = renderMinecraftParrot;
exports.renderMinecraftCampfire = renderMinecraftCampfire;
exports.renderMinecraftChest = renderMinecraftChest;
exports.renderSeasonalJackOLantern = renderSeasonalJackOLantern;
exports.renderHalloweenGhosts = renderHalloweenGhosts;
exports.renderSeasonalHolidayGifts = renderSeasonalHolidayGifts;
exports.renderSeasonalFairyLights = renderSeasonalFairyLights;
const palettes_1 = require("./palettes");
function renderMinecraftGround(width, height, groundY, isSnow = false, oreBlocks = [], treeType = "oak") {
    const terrain = palettes_1.BIOME_TERRAINS[treeType] || palettes_1.BIOME_TERRAINS.oak;
    ...
    for (const ore of oreBlocks) {
      ...
      `;
            ...
            const palette = palettes_1.ORE_PALETTES[ore.type] || palettes_1.ORE_PALETTES.diamond;
...
function renderMinecraftLog(x, y, size, treeType = "oak") {
    const pixelSize = size / 16;
    const palette = palettes_1.LOG_PALETTES[treeType] || palettes_1.LOG_PALETTES.oak;
    let rects = "";
    for (let r = 0; r < 16; r++) {
        const row = palettes_1.LOG_16X16[r];
        for (let c = 0; c < 16; c++) {
...
function renderMinecraftLeaf(leaf, frameIndex, totalFrames, isSnow = false, treeType = "oak") {
    const { x, y, size, commitLevel, gridY } = leaf;
    const pixelSize = size / 16;
    const biomePalettes = palettes_1.LEAF_PALETTES[treeType] || palettes_1.LEAF_PALETTES.oak;
    ...
    let rects = "";
    for (let r = 0; r < 16; r++) {
        const row = palettes_1.LEAF_16X16[r];
        for (let c = 0; c < 16; c++) {
...
function renderFlowerOnGrass(flower, frameIndex) {
...
function renderApple(apple, frameIndex) {
...
function renderGoldenAppleOnGrass(apple, frameIndex) {
    const { x, y, size } = apple;
...
function renderMinecraftBee(baseX, baseY, frameIndex, totalFrames) {
...
function renderMinecraftBeehive(x, y, _side = "right") {
    return `
    <!-- Minecraft Beehive -->
    <g shape-rendering="crispEdges">
...
function renderMinecraftSignpost(x, y, streak) {
...
function renderMinecraftFarmer(farmer, frameIndex, _totalFrames = 20) {
...
function renderMinecraftWolf(pet, frameIndex) {
    const { x, y } = pet;
    const ps = 1.3;
...
function renderMinecraftFox(pet, frameIndex, isNight) {
    const { x, y, state } = pet;
    const ps = 1.3;
...
function renderMinecraftCat(pet, frameIndex) {
    const { x, y } = pet;
    const ps = 1.2;
...
function renderMinecraftParrot(pet, frameIndex) {
    const { x, y } = pet;
...
function renderMinecraftCampfire(campfire, frameIndex, totalFrames) {
    const { x, y } = campfire;
...
function renderMinecraftChest(chest, frameIndex) {
    const { x, y, type } = chest;
    const isEnder = type === "ender";
    const isDiamond = type === "diamond";
    const isGold = type === "gold";
    const p = palettes_1.CHEST_PALETTES[type] || palettes_1.CHEST_PALETTES.wood;
    // Latch rendering
    let latchSvg = "";
    if (isEnder) {
...
function renderSeasonalJackOLantern(jack, frameIndex) {
    const { x, y } = jack;
...
function renderHalloweenGhosts(jackX, jackY, frameIndex, totalFrames) {
    let ghostsSvg = "";
    const ghostCount = 3;
...
function renderSeasonalHolidayGifts(gifts, _frameIndex) {
    let res = "";
...
function renderSeasonalFairyLights(leafBlocks, frameIndex) {
