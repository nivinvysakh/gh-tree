...
export interface GifInfo {
  ...
  durationMs: number;
  delaysMs: number[];
  loopCount: number | null; // null = unspecified, 0 = infinite loop
 ...
 */
export function decodeGifInfo(data: Uint8Array | ArrayBuffer | Buffer): GifInfo {
  const bytes = data instanceof Uint8Array ? data : new Uint8Array(data);

  const fallback: GifInfo = {
    valid: false,
    version: "unknown",
    width: 0,
    height: 0,
  ...
  if (bytes.length < 13) return fallback;
  ...
  const version = signature as "GIF87a" | "GIF89a";
